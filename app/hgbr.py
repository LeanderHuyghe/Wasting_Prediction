...
def hgbr_semiyearly(data_path, df_csv_name, start_time, validation_hgbr, model_hgbr, output_path):
    ...
    """
    print("Running HGBR model on semiyearly data...")
    ...
    X = df.select_dtypes(exclude=["object", "category"]).iloc[:len(y)].drop(
        ["next_prevalence", "increase_numeric", "GAM Prevalence", "Under-Five Population"], axis=1)
    ...
    if validation_hgbr == 1:
        ...
        best_model_columns = ['SAM Prevalence', 'ipc', 'rainfall', 'ndvi', 'Total alarms',
         'conflicts', 'prevalence_6lag', 'month', 'district_encoded']
        best_model_trees = 31
        best_model_depth = 1
